 ...
 *
 * The content is privacy-safe by construction (stage markers + transport error
 * types only — see [BootDiagnostics]); there is nothing here to redact before
 * sharing.
 */
@Composable
    ...
    Column(
        ...
        Text(
            text = "A local record of the app's attempts to register and connect to the " +
                "relay. Privacy-safe: stage names and error types only — never message " +
                "content, keys, or your account ID. Copy it into a bug report if you're " +
                "stuck on “Connecting…”.",
